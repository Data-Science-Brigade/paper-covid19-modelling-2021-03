...
RETROACTIVE_FOLDER = "retroactive_data"
...
def copy_retroactive_file(date):
  fname = "retroactive_{date_str}.csv".format(
    date_str=date.strftime("%Y_%m_%d"),
  )
  shutil.copy(os.path.join(RETROACTIVE_FOLDER,fname), "retroactive.csv")
...
START_OPTS = "-m DEVELOP -k 7 -i 3000 -w 2400 -c 1 -t 8 -r '{date_str}' -v TRUE >'{date_str}.out'"
SUBSEQ_OPTS = "-m DEVELOP -k 7 -i 1200 -w 600 -c 1 -t 8 -r '{date_str}' -e {previous_model_file} -v TRUE >'{date_str}.out'"
...
if __name__=="__main__":
  ...
  for data_date in daterange(start_date, end_date, 7):
      ...
      copy_retroactive_file(data_date)
      clean_deaths()
      # Run model
      subprocess.call(
      #os.system(
