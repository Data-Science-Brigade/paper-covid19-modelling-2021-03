...
get_stan_data_for_location <- function(location_name, population, IFR, N2, ecdf.saved, covid_data, common_interventions){
  ...
  # TODO: Read from file
  retroactive_df <- readr::read_csv("retroactive.csv")
  retroactive_df <- retroactive_df %>%
    mutate(region=paste0("SC_MAC_", gsub(" ", "_", region))) %>%
    filter(region==location_name)

  get_at_or_zero <- function(idx, what) {
    df <- retroactive_df %>% filter(day==idx)
    values <- df[[what]]
    ifelse(length(values)>0,values[[1]],0.0)
  }

  retroactive_cases <- 1.0 + unlist(map(0:7,function(x){get_at_or_zero(x,"casos_delta_pct_avg")}))
  retroactive_deaths <- 1.0 + unlist(map(0:7,function(x){get_at_or_zero(x,"obitos_delta_pct_avg")}))
...
wohoo <- function(){
  ...
  for(location in model_output$stan_list$available_locations){
    ...
    data <- bind_rows(data,
                      data.frame(
                        list(
                          location = rep(location,dim(model_output$stan_list$stan_data$X)[2]),
                          x = model_output$stan_list$stan_data$x,
                          i1 = model_output$stan_list$stan_data$X[i,,1],
                          i2 = model_output$stan_list$stan_data$X[i,,2],
                          i3 = model_output$stan_list$stan_data$X[i,,3],
                          i4 = model_output$stan_list$stan_data$X[i,,4],
                          i5 = model_output$stan_list$stan_data$X[i,,5],
                          i6 = model_output$stan_list$stan_data$X[i,,6]
                        )
                      ))
    i <- i+1
  }
